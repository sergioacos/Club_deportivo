...
{
    public partial class sociosDeuda : Form
    {
        public sociosDeuda()
        {
            InitializeComponent();
        ...
        public void CargarGrilla()
        {
            ...
            try
            {
                ...
                query = "select p.apellido, p.nombre, p.dni, p.fechaNac, p.aptoFisico, s.idSocio, c.fechaVencimiento,c.fechaPago from persona p inner join socio s on p.idPersona = s.idPersona inner join cuota c on c.idSocio = s.idSocio order by p.apellido";
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        bool fechaPago = reader.IsDBNull(7);
                        DateTime fechavenc = reader.GetDateTime(6);
                        
                        DateOnly fechaAct = DateOnly.FromDateTime(fechaActual);
                        DateOnly fechaVe = DateOnly.FromDateTime(fechavenc.Date);
                        if ((fechaAct == fechaVe)&(reader.IsDBNull(7))) { 
                        ...
                        dgvClientes.Rows[renglon].Cells[4].Value = reader.GetBoolean(4);
                            ...
                            dgvClientes.Rows[renglon].Cells["id"].Value = reader.GetInt32(5); //? reader.IsDBNull(6) ? 0 : reader.GetInt32(6) : reader.GetInt32(5);

                        //if((DateTime)dgvClientes.Rows[renglon].Cells[6].Value < fechaActual)
